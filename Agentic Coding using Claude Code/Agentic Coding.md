...
*   **Concept:** Automate repetitive workflows by saving instructions as Markdown files interpreted as CLI commands.
*   **Scoping:**
    *   **Project-Scoped:** `project_root/.claude/commands/*.md` (Specific to one project).
    *   **User-Scoped:** `~/.claude/commands/*.md` (Global availability across all projects).
*   **Registry:** Claude Code auto-discovers these files → requires restart to register new commands.
*   **Naming:** Filename (e.g., `seed-user.md`) → `/seed-user` command.

*   **Implementation Structure (.md file):**
    *   **Description:** Header or text at top (displayed in command help).
    *   **Allowed Tools:** Explicitly list capabilities (e.g., `read_file`, `bash`).
    *   **Instructions:** Detailed, step-by-step natural language prompt.
    *   **Variables:** Use `$arguments` to handle user input.

*   **Command Examples:**
    *   `/seed-user`: Populates database with dummy data; uses `python3` via `bash` tool.
    *   `/seed-expense [userId] [count] [months]`: Demonstrates handling multi-argument inputs.
    *   `/create-spec [stepNumber] [featureName]`: Generates a technical specification document.

*   **Automated Development Workflow:**
    1.  **Git State:** Ensure `git status` is clean before starting.
    2.  **Spec Creation:** Use `/create-spec` → generates file in `.claude/specs/[step]/[name].md`.
    3.  **Branching:** Automated `git checkout -b [feature-name]`.
    4.  **Planning:** Use **Plan Mode** to read the generated spec and draft a technical plan.
    5.  **Execution:** Confirm/approve edits to apply changes.
    6.  **Verification:** Test against defined **Acceptance Criteria**.
    7.  **Closure:** Merge and delete feature branches.
